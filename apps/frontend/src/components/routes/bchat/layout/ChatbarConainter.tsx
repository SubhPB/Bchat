        ...
        Array.from('ABCDEFGHIJKLMNOPQRS').map(
          (l) => (
            <div key={l} className='w-full flex gap-2'>
              <Skeleton className='h-16 w-16 sm:h-14 sm:w-14 rounded-full bg-gray-400'/>
              <div className='space-y-2 flex-grow'>
                <Skeleton className='w-[90%]  h-4 rounded-2xl bg-gray-400'/>
                <Skeleton className='w-[75%] max-w-[300px] h-3 rounded-xl bg-gray-400'/>
              </div>
            </div>
          )
        )
